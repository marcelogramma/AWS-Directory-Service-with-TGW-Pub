...
# Script para ejecutar terraform apply con los perfiles configurados
...
# Verificar si existe el archivo de plan
if [ ! -f terraform_plan.out ]; then
  echo "No se encontró el archivo de plan terraform_plan.out"
  echo "¿Deseas ejecutar terraform apply directamente? (s/n)"
  read respuesta
  if [[ "$respuesta" == "s" || "$respuesta" == "S" ]]; then
    ...
    echo "Ejecuta primero ./run_terraform_plan_profiles.sh para generar el plan"
    exit 1
  fi
else
...
# Verificar si terraform apply se ejecutó correctamente
if [ $? -eq 0 ]; then
...
# Ejecutar el script para corregir las rutas del Transit Gateway si es necesario
if [ -f ./fix_transit_gateway_routes.sh ]; then
  echo "ATENCION: Ejecutar SOLAMENTE si el Transit Gateway no se ha creado correctamente"
  echo "¿Deseas ejecutar el script para corregir las rutas del Transit Gateway? (s/n)"
  read respuesta
  if [[ "$respuesta" == "s" || "$respuesta" == "S" ]]; then
