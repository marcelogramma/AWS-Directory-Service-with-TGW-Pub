...
# Script para ejecutar terraform plan con los perfiles configurados
...
# Ejecutar terraform plan y guardar el resultado en un archivo
terraform plan \
...
# Verificar si terraform plan se ejecutó correctamente
if [ $? -eq 0 ]; then
  echo "Plan de Terraform generado correctamente. Para aplicar los cambios, ejecuta:"
  echo "./run_terraform_apply_profiles.sh"
else
  echo "Error al generar el plan de Terraform"
  exit 1
fi
