...
func TestStageModule(t *testing.T) {
	...
	assert.Contains(t, output, "aws_vpc.vpc_stage")
	assert.Contains(t, output, "aws_subnet.subnet_stage")
	assert.Contains(t, output, "aws_ec2_transit_gateway_vpc_attachment.tgw_attachment_stage")
	assert.Contains(t, output, "aws_route_table.rt_stage")
