...
func TestDevModule(t *testing.T) {
	// Configuración de Terratest
	terraformOptions := terraform.WithDefaultRetryableErrors(t, &terraform.Options{
		TerraformDir: "../modules/dev",
	...
	assert.Contains(t, output, "aws_vpc.vpc_dev")
	assert.Contains(t, output, "aws_subnet.subnet_dev")
	assert.Contains(t, output, "aws_ec2_transit_gateway_vpc_attachment.tgw_attachment_dev")
	assert.Contains(t, output, "aws_route_table.rt_dev")
